...
public class LoadAuditEquipmentByIdForExpertMode {
    AuditEquipmentRepository repository;
    @Inject
    public LoadAuditEquipmentByIdForExpertMode(AuditEquipmentRepository repository) {
        this.repository = repository;
    }
    public Single<AuditEquipmentModel> execute(int id){
        return Single.fromObservable(repository.loadAuditEqById(id));
    }
}
