...
/** see {@link TutorialDisplayPresenter#loadOnboarding(UseCaseCallback)} */
public class TutorialDisplayPresenterLoadOnboardingTest {
    ...
    public void shouldDelegateTaskExecutionToHandler() {
        ...
        UseCase<CompleteOnboardingRequest, CompleteOnboardingTask.CompleteOnboardingResponse> completeUseCase = mock(UseCase.class);
        ...
        // when
        subject.loadOnboarding(inputCallback);

        // then
        verify(useCaseHandler).execute(eq(loadUseCase), any(LoadOnboardingItemsRequest.class), any(UseCaseCallback.class));
    }
}